...
%' <<toMSnSet_symbols, message=FALSE>>=
%' library("org.Ce.eg.db")
%' mp <- as.list(org.Ce.egSYMBOL[as.character(prj$entrezgene)])
%' prj$accession <- sapply(mp, '[', 1)
%' mset2 <- as(prj, "MSnSet")
%' # take care of pheno data
%' sampleNames(mset2) <- sub('.mzML', '', sampleNames(mset2), fixed=T)
%' pData(mset2) <- meta[sampleNames(mset2),]
%' sampleNames(mset2) <- pData(mset2)$sampleName
%' # roll-up to gene level
%' mset2 <- combineFeatures(mset2, groupBy = fData(mset2)$accession,
%'                         fun = 'sum', cv=F, redundancy.handler = 'unique')
%' @
